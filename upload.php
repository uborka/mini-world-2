...
// Settings
	$save_path = $_SERVER["DOCUMENT_ROOT"] . "/items/temp/";				// The path were we will save the file (getcwd() may not be reliable and should be tested in your environment)
...
//	$file_name = session_id() . "_";
//	$file_name_addon = "001";
//	while (file_exists($save_path . $file_name . $file_name_addon))
//	{
//		$file_name_addon = sprintf("%03d",($file_name_addon + 1));
//	}
//	$file_name .= $file_name_addon;
...
// Process the file
	...
	*/
	...
	if (!@move_uploaded_file($_FILES[$upload_name]["tmp_name"], $save_path . $file_name . ".temp"))
	{
	...
	{
		// TODO A kép arányos lekicsinítése, illetve a miniatűr elkészítése
		ResizeImageFile($save_path . $file_name . ".temp",
			$save_path . $file_name,
			640, 480);
		ResizeImageFile($save_path . $file_name . ".temp",
			$save_path . $file_name . "_small",
			160, 120);
		unlink($save_path . $file_name . ".temp");
	}
...
function ResizeImageFile($original_img,$target_img,$max_width,$max_height)
{
    ...
    if ($type<3)
	{
		...
		{
		case 1:
			$src = imagecreatefromgif($original_img);
			break;
		case 2:
			$src = imagecreatefromjpeg($original_img);
			break;
		case 3:
			$src = imagecreatefrompng($original_img);
	    	break;
	    }
	}
	...
	// A kép generálása és mentése
	imagejpeg($img,$target_img);
